const { fodderTag } = require('~/models')
...
module.exports = {
  async create (data, ctx) {
    try {
      ...
      const { fodderId, tagIds } = data
      await this.delete({ fodderId }, ctx)
      let count = 0
      let message = ''
  ...
  async delete (where, ctx) {
    try {
      ...
      const count = await fodderTag.destroy({ where })
      if (count > 0) {
  ...
  async info (where, ctx) {
    try {
      ...
      const result = await fodderTag.findOne({ where })
      if (result) {
