...
module.exports = {
  async list (data, ctx) {
    try {
      ...
      const result = await shareLog.findAndCountAll({
        ...data,
        where: {
  ...
  async update (data, where = {}, ctx) {
    try {
      ...
      const [count = 0] = await shareLog.update(data, { where })
      if (count > 0) {
  ...
  async delete (where, ctx) {
    try {
      ...
      const [count = 0] = await shareLog.destroy({ where })
      if (count > 0) {
        return util.format.sucHandler({ count })
      } else {
  ...
  async info (where, ctx) {
    try {
      ...
      const result = await shareLog.findOne({ where })
      if (result) {
        return util.format.sucHandler(result)
      } else {
